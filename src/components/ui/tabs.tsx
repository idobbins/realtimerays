...
const TabsListVariantContext = React.createContext<IndicatorVariant>("default");
...
const tabsListVariants = cva(
  "group/tabs-list relative isolate inline-flex w-fit items-center justify-center rounded-lg p-[3px] text-muted-foreground group-data-horizontal/tabs:h-8 group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col data-[variant=line]:rounded-none",
  {
    variants: {
      variant: {
        default: "bg-muted",
        chrome: "h-8 gap-1 rounded-none bg-transparent p-0 text-muted-foreground",
...
function TabsList({
  ...
  return (
    ...
    >
      ...
      <TabsListVariantContext.Provider value={indicatorVariant}>
        {children}
  ...
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        ...
        variant === "chrome" &&
          "h-8 w-56 flex-none justify-start rounded-lg px-3 text-xs font-medium text-muted-foreground hover:text-foreground data-active:text-foreground",
